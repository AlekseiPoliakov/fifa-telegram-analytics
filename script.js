...
// В вашем классе App при клике на вкладку "Аналитика":
async showAIStats(teamId) {
    const aiData = await fetch(`/api/ai-analyze?teamId=${teamId}`).then(r => r.json());
    document.querySelector('.dashboard-content').innerHTML = `
        <div class="ai-card">
            <h3>Прогноз ИИ</h3>
            <p>${aiData.prediction}</p>
            <small>${aiData.reason}</small>
        </div>
    `;
}
